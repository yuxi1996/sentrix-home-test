...
算法服务：`http://192.168.0.200:8091`

| 部分 | 主要职责 | 当前访问/代码位置 |
|---|---|---|
| 算法服务 | 媒体导入、图片/视频分析、Embedding、人脸聚类、事件、记忆查询、实体和关系 | 200 的 Sentrix API；工程通常位于 `/home/sscy/GitHub/hpq/sentrix-home-web` 的 backend/及 scripts/ |
| 前端主界面 | 实际记忆相册、家庭记忆对话、事件时间线、人物与关系、实体知识、资料库和故事展示 | <http://192.168.0.200:4174/#/search>；200 工程的 `src/` |
| 测试平台 | 选择 QA、模型和运行模式，执行 Agent/Judge，保存逐题结果和指标 | <http://192.168.0.153:8771/>；本仓库的 `backend/`、`frontend/` |

项目只负责测评编排和结果展示，不替代 Sentrix 后端，也不负责模型训练。一次完整测评通常包含：

```mermaid
flowchart LR
    A[相册/媒体] --> B[算法服务 200:8091]
    B --> C[前端主界面 200:4174]
...
因此，优化调试不能只看 153 测试平台的分数：应在 200 主界面确认照片、人物、事件和关系确实发生了预期变化，再用 153 测试平台确认变化没有造成整体质量退化。
...
项目有两个需要区分的界面：200 的 Sentrix Home 是实际产品主界面，153 的 PhotoBench 是测试平台。

### 3.1 前端主界面：记忆相册和家庭记忆

打开：<http://192.168.0.200:4174/#/search>
...
记忆查询架构和图关系优化后的效果，主要在 `#/search`、`#/library`、`#/timeline`、`#/people` 和 `#/knowledge` 查看。
...
| 完整记忆相册 | 200 主界面 `#/library` | 浏览真实 Asset，按媒体类型筛选，打开原图、视频和处理详情 |
| 事件记忆 | 200 主界面 `#/timeline` | 查看事件时间、地点、照片、视频场景和原始证据 |
| 人物和关系 | 200 主界面 `#/people`、`#/knowledge` | 查看人物样本、实体画像、关系图和待确认关系 |
| 自然语言查询 | 200 主界面 `#/search` | 查看回答、检索到的照片、证据和查询过程 |
...
200 主界面已经提供记忆相册的实际查看入口，尤其是 `#/library` 资料库、`#/timeline` 事件时间线、`#/people` 人物与关系和 `#/knowledge` 实体与知识。153 测试平台负责测评，不是完整的记忆相册主界面。观察优化效果时，应先在 200 主界面确认可视结果，再在 153 测试平台确认量化结果。
...
- Sentrix 算法服务 API 地址，当前主界面对应 `http://192.168.0.200:8091`；测试时以面板实际配置为准；
...
```bash
curl 'http://192.168.0.200:8091/api/memory-spaces?limit=1000'
```

再查询该 scope 的资产：

```bash
curl 'http://192.168.0.200:8091/api/assets?scope_id=<SCOPE_ID>&limit=2000'
```

单个照片或视频资产可通过返回的 `asset.id` 查看：

```text
http://192.168.0.200:8091/api/assets/<ASSET_ID>/file
```
...
## 5. 算法服务、主界面与 153 测试数据访问方式

200 上的算法服务和前端主界面工程：

```text
/home/sscy/GitHub/hpq/sentrix-home-web       # 200 算法服务与前端主界面工程
```

200 服务访问：

```bash
ssh sscy@192.168.0.200
cd /home/sscy/GitHub/hpq/sentrix-home-web
...
算法服务当前 `search_memories` 的主要逻辑如下：
